...
struct HNode* CreateNode(int val)
{
...
{
    struct HNode* newNode = CreateNode(v);

    hGenericInsert(list -> head -> prev, newNode, list -> head);
}

struct HNode* HSearch(struct HList* list, int v)
{
    struct HNode* temp = list -> head -> next;
...
struct VNode* createNode(int val)
{
...
void InitVList(struct VList* vlist)
{
    vlist -> head = createNode(-1);

    vlist -> head -> next = vlist -> head;
    vlist -> head -> prev = vlist -> head;
}
...
{
    struct VNode* newNode = createNode(v);

    vGenericInsert(list -> head -> prev, newNode, list -> head);
}

struct VNode* VSearch(struct VList* list, int v)
{
    struct VNode* temp = list -> head -> next;
...
void AddVertex(struct Graph *G, int v)
{
    ...
    {
        printf("Vertex %d already Exists !!!",v);
        return;
    }
...
void RemoveVertex(struct Graph* G, int v)
{
    ...
    while(run != r -> adjList.head)
    {
        ...
        struct VNode* adj = VSearch(&G->verticalList, run -> val);
